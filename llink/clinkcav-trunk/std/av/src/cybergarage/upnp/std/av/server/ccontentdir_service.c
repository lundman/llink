...
struct sort_node_struct {
    ...
    CgUpnpAvResource *data;
};

static int sortby_size(const void *data1, const void *data2)
{
    if ( ((struct sort_node_struct *)data1)->size >=
         ((struct sort_node_struct *)data2)->size)
        return -1;
    else
        return 1;
}
...
{
    if ( ((struct sort_node_struct *)data2)->size >=
         ((struct sort_node_struct *)data1)->size)
        return -1;
    else
        return 1;
}
...
{
    /* Luckily, cg_strcmp handles NULLs. Yay */
    return (cg_strcmp(
                   ((struct sort_node_struct *)data1)->str,
                   ((struct sort_node_struct *)data2)->str)
            );
}
...
void cg_upnpav_content_sort(int total,
                            CgUpnpAvContent **sortedContentArray,
                            char *criteria)
{
#if HAVE_QSORT
    int (*sorter)(const void *, const void *) = NULL;
    struct sort_node_struct *sort_array = NULL;
    int n;
    int reverse = 0;

    /* Valid SearchCriteria may include: "+dc:date", "-dc:title" etc.
     * And chained, "+upnp:class,-dc:date". Ie, Directories first,
     * newest first within those regions.
     * Some examples handles 3 and more chained. One day we could too. */
    if ((total <= 1) ||
        !sortedContentArray ||
        !criteria ||
        !criteria[1]) return;

    sort_array = calloc(sizeof(struct sort_node_struct), total);
    if (!sort_array) return;
    ...
    if (cg_streq(&criteria[1], "res@size")) {

        sorter = reverse ? sortby_size : sortby_size_reverse ;

        /* Load the array up with size */
        for (n = 0; n < total; n++) {
            CgUpnpAvResource *r;
            char *str1 = NULL;
            r = cg_upnpav_content_getresources(sortedContentArray[n]);
            if (r)
                str1 = cg_xml_node_getattributevalue(r,
                                                     CG_UPNPAV_RESOURCE_PROTOCOLINFO_SIZE);
            if (str1)
                sort_array[n].size = strtoull(str1, NULL, 10);

            sort_array[n].data = sortedContentArray[n];
        }
    } else if (cg_streq(&criteria[1], "dc:title")) {

        sorter = reverse ? sortby_name : sortby_name_reverse ;

        /* Load the array up with title */
        for (n = 0; n < total; n++) {
            sort_array[n].str  = cg_upnpav_content_gettitle(sortedContentArray[n]);
            sort_array[n].data = sortedContentArray[n];
        }

    } else if (cg_streq(&criteria[1], "dc:date")) {

        sorter = reverse ? sortby_name : sortby_name_reverse ;

        /* Load the array up with date, we use string compare here
         * instead of converting to time_t */
        for (n = 0; n < total; n++) {
            sort_array[n].str  = cg_xml_node_getchildnodevalue(sortedContentArray[n],
                CG_UPNPAV_OBJECT_DATE);
            sort_array[n].data = sortedContentArray[n];
        }
    } else if (cg_streq(&criteria[1], "upnp:class")) {

        sorter = reverse ? sortby_name : sortby_name_reverse ;

        /* Load the array up with class, a string */
        for (n = 0; n < total; n++) {
            sort_array[n].str  = cg_upnpav_content_getupnpclass(sortedContentArray[n]);
            sort_array[n].data = sortedContentArray[n];
        }
    }
    ...
    /* Actually sort. */
    qsort((void *)sort_array,
          total,
          sizeof(struct sort_node_struct),
          sorter);

    /* Copy sorted nodes back, now in the correct order */
    for (n = 0; n < total; n++) {
        sortedContentArray[n] = sort_array[n].data;
    }
...
static BOOL cg_upnpav_dms_condir_browsedirectchildren(CgUpnpAvServer *dms, CgUpnpAction *action)
{
	...
	*/
    ...
    /* If sorting requested, do so now */
    if (cg_upnp_action_getargumentvaluebyname(action,CG_UPNPAV_DMS_CONTENTDIRECTORY_BROWSE_SORT_CRITERIA))
        cg_upnpav_content_sort(n, sortedContentArray,
                               cg_upnp_action_getargumentvaluebyname(action,CG_UPNPAV_DMS_CONTENTDIRECTORY_BROWSE_SORT_CRITERIA));
...
BOOL cg_upnpav_dms_condir_actionreceived(CgUpnpAction *action)
{
	...
	if (cg_streq(actionName, CG_UPNPAV_DMS_CONTENTDIRECTORY_GET_SORT_CAPABILITIES)) {
		...
		if (!arg)
			return FALSE;
#if HAVE_QSORT
		cg_upnp_argument_setvalue(arg, "dc:title,dc:date,upnp:class,res@size");
#else
...
BOOL cg_upnpav_dms_medrec_actionreceived(CgUpnpAction *action)
{
    ...
    if (cg_streq(actionName, CG_UPNPAV_DMS_MEDIARECEIVER_IS_AUTHORIZED )) {
        ...
        cg_upnp_action_setargumentvaluebyname(action,
                                              CG_UPNPAV_DMS_MEDIARECEIVER_RESULT,
                                              "1");

...
static CgNetworkInterfaceList *netAllowedInterfaces = NULL;
...
int cg_net_gethostinterfaces(CgNetworkInterfaceList *netIfList)
{
    ...
    for (i = 0; i < nNumInterfaces; ++i) {
		...
		//	continue;
        /* Ordinarily we use all interfaces we found that are appropriate for
         * UPNP. However, if the "allowedInterface list is set, we only
         * accept those that match.
         */
        if (netAllowedInterfaces &&
            !cg_net_interfacelist_get(netAllowedInterfaces, InterfaceList[i].iiAddress))
            continue;
	...
	if ((dwRetVal = GetAdaptersInfo( pAdapterInfo, &ulOutBufLen)) == NO_ERROR) {
		for (pAdapter = pAdapterInfo, nOfInterfaces = 0; pAdapter; pAdapter = pAdapter->Next, ++nOfInterfaces) {
             ...
             */
            if (netAllowedInterfaces &&
                !cg_net_interfacelist_get(netAllowedInterfaces, pAdapter->AdapterName))
                continue;
	...
	while (ai != NULL) {
         ...
         */
        if (netAllowedInterfaces &&
            !cg_net_interfacelist_get(netAllowedInterfaces, ai->AdapterName))
            continue;
...
int cg_net_gethostinterfaces(CgNetworkInterfaceList *netIfList)
{
	...
	for (i = ifaddr; i != NULL; i = i->ifa_next) {
         ...
         */
        if (netAllowedInterfaces &&
            !cg_net_interfacelist_get(netAllowedInterfaces, i->ifa_name))
            continue;

#if 1
        // Some implementations of "tap" interfaces do not set af_family
        // for the "ifa_netmask" type, and is indeed left as AF_UNSPEC (0).
        // However, sometimes it is nice to enable 'tap' interfaces too.
        // We already know ifa_addr->sa_family is AF_INET above.
        // - Lundman
        if (i->ifa_netmask->sa_family == AF_UNSPEC)
            i->ifa_netmask->sa_family  = AF_INET;
#endif

		if ((err=getnameinfo(i->ifa_netmask, sizeof(struct sockaddr), netmask, NI_MAXHOST, NULL, 0, NI_NUMERICHOST)) != 0)
            continue;
...
int cg_net_gethostinterfaces(CgNetworkInterfaceList *netIfList)
{
	...
	while (!feof(fd)) {
         ...
         */
        if (netAllowedInterfaces &&
            !cg_net_interfacelist_get(netAllowedInterfaces, ifname))
            continue;
...
void cg_net_allow_interface(char *ifname)
{
	CgNetworkInterface *netIf;

    if (!netAllowedInterfaces)
    	netAllowedInterfaces = cg_net_interfacelist_new();

    netIf = cg_net_interface_new();
    cg_net_interface_setname(netIf, ifname);
    cg_net_interfacelist_add(netAllowedInterfaces, netIf);

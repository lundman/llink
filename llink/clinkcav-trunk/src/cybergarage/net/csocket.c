...
BOOL cg_socket_tosockaddrinfo(int sockType, char *addr, int port, struct addrinfo **addrInfo, BOOL isBindAddr)
{
    ...
    {
        struct addrinfo *res;
            cg_log_debug_s("getaddrinfo() returns: \n");
        for (res = *addrInfo; res; res = res->ai_next) {
            char hostname[200] = "";
            char poort[200] = "";

            getnameinfo(res->ai_addr, res->ai_addrlen, hostname,
                      200, NULL, 0, NI_NUMERICHOST);
            getnameinfo(res->ai_addr, res->ai_addrlen, poort,
                      200, NULL, 0, NI_NUMERICSERV);

            cg_log_debug_s("      %d %s:%s\n", res->ai_family,
                           hostname, poort);
        }
    }
#endif
